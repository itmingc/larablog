...
@section('content')
    ...
    <div class="result_wrap">
        <form action="{{url('admin/article/' . $article->id)}}" method="post" id="add-form">
            <table class="add_tab">
                <tbody>
                    <tr>
                        ...
                        <td>
                            <select name="cid" id="cid">
                                ...
                                @foreach($category as $v)
                                    ...
                                    @endif
                                    >@if($v->pid) &nbsp;&nbsp;|@endif{{$v->html}}{{$v->name}}</option>
                                @endforeach
                            </select>
                        </td>
                    </tr>
                    ...
                    <tr>
                        <th>缩略图：</th>
                        <td>
                            <input type="text" id="thumb" name="thumb" value="{{url($article->thumb)}}" readonly>
                    ...
                    <tr id="img-tr" hidden>
                        <th>&nbsp;</th>
                        <td>
                            <img src="{{$article->thumb}}" alt="上传的图片" style="max-width: 300px; max-height: 120px;">
                        </td>
                    </tr>
                    <tr>
                        <th>标签：</th>
                        <td>
                            <input type="text" name="tag" size="100" value="@foreach($article->tags as $k => $v){{$v->name}}@if($k != count($article->tags) - 1),@endif @endforeach">
                            <span><i class="fa fa-exclamation-circle yellow"></i>多个标签之间以逗号 ","分隔</span>
                        </td>
                    </tr>
                    <tr>
                        <th>描述：</th>
                        <td>
                            <input type="text" name="description" value="{{$article->description}}" size="100">
                        </td>
                    </tr>
                    <tr>
                        <th style="vertical-align: top;">文章内容：</th>
                        <td>
                            <textarea name="content" id="container">{!! $article->content !!}</textarea>
                        </td>
                    </tr>
                    <tr>
                        <th>&nbsp;</th>
